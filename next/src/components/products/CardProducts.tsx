...
const CardProducts = ({ products, manufactures }: ICardProductsProps) => {
			...
			<table className='w-full select-none cursor-pointer'>
				<tbody className='grid grid-rows-2 grid-cols-4 gap-[10px]'>
					{products.map((product) => (
						<tr
							key={product.id}
							className='grid items-start justify-center gap-y-2 text-center text-xs hover:bg-[#0f172a0f] rounded-xl 
                p-[10px] duration-150'
                ...
                <Image
							...
							<td
								title={product.name}
								className='flex flex-col break-words leading-normal text-base mb-2'
							>
								{product.name}
							</td>
							<td
