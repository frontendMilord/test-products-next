...
const TableProducts = ({ products, manufactures }: ITableProductsProps) => {
	...
	const onEditClick = (e: MouseEvent, product: IProduct) => {
		e.stopPropagation()
		setIsEditProductModalVisible(true)
		setSelectedProduct(product)
	}

	const onDeleteClick = (e: MouseEvent, product: IProduct) => {
		e.stopPropagation()
	...
	return (
		<>
			...
			<table className='p-3 w-full flex flex-col gap-y-3'>
				<thead>
					<tr className='flex justify-between py-3 pl-4'>
						<th className='w-20 text-start'>Фото</th>
						<th className='w-56'>Название</th>
						<th className='w-28'>Количество</th>
						<th className='w-44'>Производитель</th>
						<th className='w-24'>Цена</th>
						<th className='invisible w-20'></th>
					</tr>
				</thead>
				<tbody>
					{products?.map((product, index) => (
						<tr
							key={product.id}
							className={`flex justify-between items-center p-3 text-xs duration-150 rounded-md hover:bg-[#0F172A1A] 
                  ${index % 2 !== 0 && 'bg-[#0F172A08]'}
                `}
                ...
                <Image
							...
							<td className='w-24'>{product.quantity}</td>
							<td
								title={
							...
							<td className='w-24'>{product.price} р</td>
							<td className='w-14 flex gap-x-3'>
								<div
								...
								<div
									className='cursor-pointer'
									onClick={(e) => onDeleteClick(e, product)}
								>
									<svg
										width='18'
										height='22'
